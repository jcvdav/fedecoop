...
plot(n_t, type = "l")
abline(h = 25e6)
abline(h = mean(n_t), col = "blue")

```

```{r}
plot(n_t, c(NA, diff(n_t)))

log(n_t / lag(n_t)) %>% mean(na.rm = T)

# lm(log(n_t / lag(n_t)) ~ n_t) %>%
  # summary()
```


```{r}
apply(N[-1, , ], 1, function(x){apply(x, 2, sum)}) %>% 
  magrittr::set_colnames(size_class[-1]) %>% 
  as_tibble() %>% 
  mutate(t = 1:n_steps) %>% 
  gather(class, n, -t) %>% 
